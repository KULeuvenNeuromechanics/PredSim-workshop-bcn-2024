function [Results,DatStore,Misc] = solveMuscleRedundancy_BCN_workshop(time,Misc)
% -----------------------------------------------------------------------%
% INPUTS:
%           time: time window
...
Misc.NMuscles = NMuscles;

% %% Get the max EMGs % Run only once with all trials, try to include trials with heavy activation like cmj, squats
% if ~exist(fullfile(Misc.OutPath,'emgMax_all.mat'))
%     if Misc.normalizeEMG
%         getMaxEMG(Misc,DatStore);
%     end
% end
