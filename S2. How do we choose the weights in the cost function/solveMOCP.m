function MOCPsol = solveMOCP(weights,generate_animation,generate_plots,ideal,nadir)
...
%
% TWO OBJECTIVE FUNCTIONALS are concurrently minimized:
%  - J1 = time integral of the squared 2-norm of joint torques
%  - J2 = time integral of the squared 2-norm of angular accelerations
% They are combined into a single objective function using a multiobjective 
% optimization strategy, namely adopting a weighted Chebyshev metric.
%
% INPUT arguments:
% - 'weights': vector containing the 2 weights for the 2 cost functions
...
% - 'ideal': ideal objective vector (for normalization purposes)
% - 'nadir': nadir objective vector (for normalization purposes)
%
...
% Initialize the two cost functions (J1, J2).
J1 = 0;
J2 = 0;
...
for k=1:N
    ...
    % Cost functions (better, "functionals").
    % J1: minimize the sum of the squared joint torques.
    J1 = J1 + (T1k_plus.^2 + T2k_plus.^2 + T3k_plus.^2 + ...
        T4k_plus.^2 + T5k_plus.^2)*dt;
    % J2: minimize the sum of the squared angular accelerations
    % of the five segments.
    J2 = J2 + (ddq1k_plus.^2 + ddq2k_plus.^2 + ddq3k_plus.^2 + ...
...
% Use ideal and nadir points to (approximately) normalize the objectives
% between 0 and 1 over the Pareto front. Ideal and nadir points are not
% provided when single-objective minimizations are performed, that is for
...
J1 = (J1-ideal(1))/(nadir(1)-ideal(1));
J2 = (J2-ideal(2))/(nadir(2)-ideal(2));

% Smooth (and exact) reformulation of the weighted Chebyshev problem:
% minimize max(w1*J1,w2*J2)
opti.subject_to(s >= weights(1)*J1);
opti.subject_to(s >= weights(2)*J2);
opti.minimize(s);

% Create an NLP solver (IPOPT).
% To completely suppress IPOPT verbosity (if needed), set both
% 'print_time' and 'print_level' to 0.
p_opts = struct('print_time',0); % default 1
s_opts = struct('print_level',0); % default 5
...
T4_opt = sol.value(T4);
T5_opt = sol.value(T5);
% Cost functions at optimal solution.
J1_opt = sol.value(J1);
J2_opt = sol.value(J2);
...
MOCPsol.J1_normalized_opt = J1_opt;
MOCPsol.J2_normalized_opt = J2_opt;
MOCPsol.J1_original_opt = J1_opt*(nadir(1)-ideal(1))+ideal(1);
MOCPsol.J2_original_opt = J2_opt*(nadir(2)-ideal(2))+ideal(2);
