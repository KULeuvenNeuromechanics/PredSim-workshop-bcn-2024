function [varargout] = add_track_sim_to_batch(S,osim_path,varargin)
% --------------------------------------------------------------------------
% add_track_sim_to_batch
%   This functions runs the simulation as a batch job. Doing so
...
% Modified by Gil Serrancolí, 2024
% --------------------------------------------------------------------------

if isempty(varargin)
    TrackSim_fun = 'run_track_sim';
else
    TrackSim_fun = varargin{1};
end
...
% Add job to batch
batch(myCluster,TrackSim_fun,0,{S,osim_path},'CurrentFolder',S.misc.main_path,...
